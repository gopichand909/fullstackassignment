...
class Product(Base):
    ...
    category = Column(String(255), nullable=True, index=True)
    product_url = Column(String(1024), nullable=False)         # From CSV 'product_url'
    original_source = Column(String(1024), nullable=True, index=True)  # URL used for deduplication
    main_images = Column(JSON, nullable=True) 
...
class PriceHistory(Base):
    ...
    price = Column(Float, nullable=False)
    source = Column(String(1024), nullable=True)  # Marketplace URL that reported this price
    timestamp = Column(DateTime, default=datetime.utcnow, nullable=False)

    product = relationship("Product", back_populates="price_history")


# API Key, Request Logging & Webhook models (unchanged)
class ApiKey(Base):
    __tablename__ = "api_keys"
    id = Column(Integer, primary_key=True, autoincrement=True)
    key = Column(String(64), unique=True, nullable=False, index=True)
...
class Webhook(Base):
    ...
    def __repr__(self) -> str:
>>>>>>> 15c7797 (Initial commit: project setup)
